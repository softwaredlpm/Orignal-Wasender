...
class MessageMedia {
    ...
    /**
     * Helper to get MIME type safely across different versions of 'mime' (v1, v2, v3, v4)
     */
    static getMime(filePathOrUrl) {
        if (!filePathOrUrl) return 'application/octet-stream';
        if (typeof mime.getType === 'function') return mime.getType(filePathOrUrl);
        if (typeof mime.lookup === 'function') return mime.lookup(filePathOrUrl);
    ...
    static fromFilePath(filePath) {
        ...
        const mimetype = MessageMedia.getMime(filePath);
    ...
    static async fromUrl(url, options = {}) {
        ...
        let mimetype = MessageMedia.getMime(pUrl.pathname);
