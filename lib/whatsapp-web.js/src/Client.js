                    ...
                    try {
                        ...
                        if (!injected) {
                            ...
                            await this.pupPage
                                ...
                                .catch(() => {
                                    console.warn('⚠️ [whatsapp-web.js] WWebJS inject timeout; attempting anyway');
                                });
                            ...
                            let clientInfoData = {};
                            try {
                                clientInfoData = await this.pupPage.evaluate(() => {
                                    const connMod = window.require('WAWebConnModel');
                                    const meMod = window.require('WAWebUserPrefsMeUser');
                                    return {
                                        ...(connMod?.Conn?.serialize ? connMod.Conn.serialize() : {}),
                                        wid:
                                            meMod?.getMaybeMePnUser?.() ||
                                            meMod?.getMaybeMeLidUser?.() ||
                                            connMod?.Conn?.wid ||
                                            null,
                                    };
                                });
                            } catch (e) {
                                console.warn('⚠️ [whatsapp-web.js] Error fetching ClientInfo:', e.message);
                            }

                            this.info = new ClientInfo(this, clientInfoData);
                            this.interface = new InterfaceController(this);

                            // Robustly attach event listeners with retry (prevents missed 'message' events if page had a brief navigation)
                            ...
                            for (let attempt = 1; attempt <= 3; attempt++) {
                                try {
                                    ...
                                    if (attempt < 3) {
                                        ...
                                        console.warn('⚠️ [whatsapp-web.js] Final retry error attaching event listeners:', e.message);
                                    }
                                }
                            }
                        }
                        ...
                        console.error('❌ [whatsapp-web.js] onAppStateHasSyncedEvent error:', syncErr);
                        // Emit ready regardless so queues are not permanently frozen in standby
                        this.emit(Events.READY);
                    }
                },
            );
        ...
        const sentMsg = await this.pupPage.evaluate(
            async (chatId, content, options, sendSeen) => {
                ...
                try {
                    ...
                    return msg ? { id: msg.id, ack: 0, from: msg.from, to: msg.to } : undefined;
                }
            },
            chatId,
            content,
